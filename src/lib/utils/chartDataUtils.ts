...
export function calculateChartDimensions(
  ...
  const yAxisLength = SVG_CHART_HEIGHT; //실제 y축의 길이
...
export function calculateYAxisBounds(max: number[], min: number[]) {
  const dataYMax = Math.max(...max); // 차트의 Y축에서 데이터의 최대 높이를 나타내며, 가장 높은 주가를 의미
  const dataYMin = Math.min(...min); // 차트의 Y축에서 데이터의 최소 높이를 나타내며, 가장 낮은 주가를 의미
